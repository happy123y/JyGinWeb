...
func (api *OSSUploadApi) UploadFile(c *gin.Context) {
	// 获取文件上传传递过来的参数
	dir := c.PostForm("dir")
	//xxx := c.PostForm("xxxx")
	// 单文件
	...
	// oss创建bucketname
	api.CreateBuketName(bucketname)
	// oss文件上传开始
	client := api.InitOssClient()
...
// 1： 创建bucketname
func (api *OSSUploadApi) InitOssClient() *oss.Client {
...
// 创建bucketname
func (api *OSSUploadApi) CreateBuketName(bucketName string) (bool, error) {
	// 2: 创建bucketname
	exist, _ := api.InitOssClient().IsBucketExist(bucketName)
	if !exist {
		err := api.InitOssClient().CreateBucket(bucketName)
		if err != nil {
